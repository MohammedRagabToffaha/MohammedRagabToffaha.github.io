...
$html="<div class='row' >";
	...
	foreach($result7 as $row)
	{
		...
		$statement4 = $connection->prepare("select * from customerspaidMony where custID=:custID and paidDate=:paidDate and remainFromLast<>0 order by customerspaidMonyID desc");
		...
		foreach($result4 as $row4){
			...
			$finalRemain=$row4['remainFromLast']-$row4['customerspaidMonyAmount'];
		...
		<label>المتبقي ســــابقا ".number_format($row4['remainFromLast'])." جنيه"."</label></th><th width='30%'><label>تنزيل مبلغ وقدره ".number_format($row4['customerspaidMonyAmount'])." جنيه</label></th>
